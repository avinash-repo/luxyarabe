export const downloadInvoice = async (fetchORderData) => {
  const getDiscountedPrice = (fetchORderData) => {
    let discountPrice = 0;
    try {

      let priceArr = [];
      console.log("333333333333",fetchORderData?.variant_quantity)
      fetchORderData?.variant_quantity.forEach((val) => {
        if (
          val?.findProductObj?.cat_id ==
            fetchORderData?.couponObj?.category_id ||
          fetchORderData?.couponObj?.category_id == null
        ) {
          const price = parseFloat(
            Number(val?.variantObj?.variant_price_details[0]?.price) -
              (Number(val?.variantObj?.variant_price_details[0]?.discount) *
                Number(val?.variantObj?.variant_price_details[0]?.price)) /
                100
          ).toFixed(2);
          for (let i = 0; i < val?.quantity; i++) {
            priceArr.push(price);
          }
        }
      });
      if (priceArr.length > 0) {
        priceArr.sort((a, b) => a - b);
      }

      for (let i = 0; i < fetchORderData?.couponObj?.get_product; i++) {
        discountPrice += Number(priceArr[i]);
      }
      return Number(discountPrice)?.toFixed(2);
    } catch (err) {
      console.log(err);
      return 0;
    }
  };
  let {
    userAddressObj,
    variant_quantity,
    taxData,
    delivery_charges,
    onlinePaymentData,
    homeDeliveryData
  } = fetchORderData;
  // console.log(homeDeliveryData,"222222222222",delivery_charges,888888888,fetchORderData)

  let productsRows = "";
  let totalPriceOfproducts = 0;
  let taxAmount = 0;
  let couponPrice = 0;
  let student_discount_price = 0;
  let total = 0;

  // console.log(Number(delivery_charges), onlinePaymentData?.discount);
  // let deviveryCharge = fetchORderData?.payment_method
  //   .getDiscountedPrice()
  //   .includes("cash")
  //   ? Number(delivery_charges)
  //   : (Number(delivery_charges) / (100 - onlinePaymentData?.discount)) * 100;
  let deviveryCharge = Number(delivery_charges);
  

  for (let index = 0; index < variant_quantity.length; index++) {
    const row = variant_quantity[index];


    let price_details = row?.variantObj?.variant_price_details?.[0];
    price_details =
      Number(price_details?.price) -
      (Number(price_details?.price) * Number(price_details?.discount)) / 100;

    // let taxablePrice =
    //   Number(price_details) -
    //   (Number(price_details) * Number(taxData?.tax_value)) / 100;
    let taxablePrice =
      (Number(price_details) / (100 + Number(taxData?.tax_value))) * 100;

    let TotalPrice = Number(taxablePrice) * Number(row?.quantity);
    
    totalPriceOfproducts += TotalPrice;
    // console.log("price_details", TotalPrice, "price_details");
    taxAmount = (
      (Number(totalPriceOfproducts) * Number(taxData?.tax_value)) /
      100
    )?.toFixed(2);
    total = (Number(taxAmount) + Number(totalPriceOfproducts))?.toFixed(2);
    couponPrice =
      fetchORderData?.couponObj?.type === "fixed"
        ? Number(fetchORderData?.couponObj?.value)?.toFixed(2)
        : (Number(total) *
            Number(fetchORderData?.couponObj?.value)) /
            100 || 0;
    
    if (fetchORderData?.is_student_info_id) {
      if(row?.is_student){
        student_discount_price = ((Number(price_details) * Number(row?.quantity))*fetchORderData?.objDelivery?.discount)/100;
      }
    }

   

    productsRows += `
    <tr style="font-size:14px; text-align:left; color:#000; font-weight:500; padding:10px 0px; border-bottom:1px solid #D7DAE0">
      <td style="padding:10px 0 10px 10px;"> 
        ${row?.findProductObj?.title} 
      </td>
      <td style="padding:10px 0 10px 10px;"> 
        ${price_details?.toFixed(2)} 
      </td>
      <td style="padding:10px 0 10px 10px;"> 
        ${taxablePrice?.toFixed(2)} 
      </td>
      <td style="padding:10px 0">
        ${row?.quantity}
      </td>
      <td style="padding:10px 10px 10px 0; text-align:right">
        ${
          variant_quantity?.[0]?.variantObj?.variant_price_details?.[0]
            ?.currency_symbol
        } ${TotalPrice?.toFixed(2)}
      </td>
    </tr>
    
    
    
    `;
  }

  // console.log("student_discount_price",student_discount_price,fetchORderData?.is_student_info_id,fetchORderData)
  let couponorstudentoffer = fetchORderData?.couponObj?.type == "buy_get"
    ? getDiscountedPrice(fetchORderData)
    : Number(couponPrice)?.toFixed(2)

  let tenpercentAmount = fetchORderData?.payment_method
    .toLowerCase()
    .includes("cash")
    ? Number(total) - couponorstudentoffer- Number(student_discount_price) + Number(deviveryCharge)
    : ((Number(total) -
        couponorstudentoffer -
        Number(student_discount_price) +
        Number(deviveryCharge)) *
        onlinePaymentData?.discount) /
      100;




  
  
  let grandAmount = !fetchORderData?.payment_method
    .toLowerCase()
    .includes("cash")
    ? Number(total) -
      Number(couponorstudentoffer) -
      Number(student_discount_price) +
      Number(deviveryCharge) -
      Number(tenpercentAmount)
     
    : Number(total) -
      Number(student_discount_price) +
      Number(deviveryCharge) -
      Number(couponorstudentoffer)

      console.log("couponPrice",couponPrice,total,fetchORderData?.delivery_charges,student_discount_price,tenpercentAmount,couponorstudentoffer,grandAmount)

  let ProductHtml = `
  <div>
    <div style="display:flex; width:100%; background-color:#fff; border-radius:10px; border:1px solid #D7DAE0; padding:20px">

      <div style="display: flex;flex-direction:column;font-size:14px;text-align:left; color:#5E6470; width:35%">
        <div>Order id:<div style="color:#000; margin-bottom:10px"> ${
          fetchORderData?.order_id
        }</div></div>
        <div> Order Date:<div style="color:#000; margin-bottom:10px"> ${fetchORderData?.created_at
          ?.toString()
          ?.slice(0, 25)}</div></div>
        <div>Delivery Date:<div style="color:#000; margin-bottom:10px"> ${fetchORderData?.delivery_date
          ?.toString()
          ?.slice(0, 25)}</div></div>        
      </div>

      <div style="display:flex;flex-direction:column; text-align:left; width:35%">
        <div style="color:#5E6470; margin-bottom:10px">Billed to</div>
        <div style="margin-bottom:3px">${
          fetchORderData?.userAddressObj?.full_name
        }</div>
        <div style="margin-bottom:3px; line-height:18px">${
          userAddressObj?.house_no?.length !== 0
            ? `${userAddressObj?.house_no},`
            : ""
        } ${userAddressObj?.address},${userAddressObj?.city},${
    userAddressObj?.state
  },${userAddressObj?.zipcode}
        </div>
        <div>${userAddressObj?.mobile}</div>      
      </div>

      <div style="display: flex;flex-direction:column;font-size:14px;text-align:right;color:#5E6470; width:30%">
        <div>Invoice<div style="color: #032140;font-size:24px;font-weight:700; margin-bottom:10px">
        ${
          variant_quantity?.[0]?.variantObj
            ?.variant_price_details?.[0]?.currency_symbol
        } ${
          grandAmount.toFixed(2)
        }</div></div>        
      </div>

    </div>


    <div style="display:flex;flex-direction:column;">
      <div> 
    
     
    <div style="margin-top: 20px;">
      <div style="display: flex; text-align:left; width: 100%">
          <table style="border-collapse: collapse; width:100%; margin: 0 auto; text-align: left;">
              <thead style="background-color: #F6F8FC;">
                  <tr style="font-size:14px; text-align:left; color:#5E6470; padding:10px 0px; border-top:1px solid #D7DAE0; border-bottom:1px solid #D7DAE0">
                      <th style="padding:10px 0 10px 10px;">Product Name</th>
                      <th style="padding:10px 0 10px 10px;">Gross Amount</th>
                      <th style="padding:10px 0 10px 10px;">Taxable Amount</th>
                      <th style="padding:10px 0px;">Quantity</th>
                      <th style="padding:10px 10px; 10px 0; text-align:right">Total Amount</th>
                  </tr>
              </thead>
              <tbody>
                ${productsRows}

                <tr style="font-size:14px; text-align:left; color:#000; font-weight:500; padding:10px 0px;">
                  <td style="padding:10px 0"> 
                  
                  </td>
                  <td style="padding:10px 0"> 
                  
                  </td>
                  <td style="padding:10px 0"> 
                  
                  </td>
                  <td style="padding:10px 0">
                      Subtotal
                  </td>
                  <td style="padding:10px 10px 10px 0; text-align:right">
                  ${
                    variant_quantity?.[0]?.variantObj
                      ?.variant_price_details?.[0]?.currency_symbol
                  } ${totalPriceOfproducts?.toFixed(2)}
                  </td>
                </tr>

                <tr style="font-size:14px; text-align:left; color:#000; font-weight:500; padding:10px 0px;">
                  <td style="padding:10px 0"> 
                  
                  </td>
                  <td style="padding:10px 0"> 
                  
                  </td>
                  <td style="padding:10px 0"> 
                  
                  </td>
                  <td style="padding:10px 0; border-bottom:1px solid #D7DAE0">
                    ${taxData?.tax_name} (${taxData?.tax_value}%)
                  </td>
                  <td style="padding:10px 10px 10px 0; border-bottom:1px solid #D7DAE0; text-align:right">
                  ${
                    variant_quantity?.[0]?.variantObj
                      ?.variant_price_details?.[0]?.currency_symbol
                  }
                    ${taxAmount}
                  </td>
                </tr>

                <tr style="font-size:14px; text-align:left; padding:10px 0px;">
                <td style="padding:10px 0"> 
                
                </td>
                <td style="padding:10px 0"> 
                
                </td>
                <td style="padding:10px 0"> 
                
                </td>
                <td style="padding:10px 0; font-weight:700;">
                  Total 
                </td>
                <td style="padding:10px 10px 10px 0;  border-bottom:1px solid #D7DAE0; color:#032140; text-align:right">
                ${
                  variant_quantity?.[0]?.variantObj?.variant_price_details?.[0]
                    ?.currency_symbol
                } ${total}
                </td>
              </tr>
    
               ${
                 fetchORderData?.is_student_info_id
                   ? `<tr style="font-size:14px; text-align:left; color:#000; font-weight:500; padding:10px 0px;">
                     <td style="padding:10px 0"></td>
                     <td style="padding:10px 0"></td>
                     <td style="padding:10px 0"></td>
                     <td style="padding:10px 0; border-bottom:1px solid #D7DAE0">
                       Student Discount(${
                         fetchORderData?.objDelivery?.discount
                       }%)
                     </td>
                     <td style="padding:10px 10px 10px 0; border-bottom:1px solid #D7DAE0; text-align:right">
                       ${
                         variant_quantity?.[0]?.variantObj
                           ?.variant_price_details?.[0]?.currency_symbol
                       }
                       ${Number(student_discount_price)?.toFixed(2)}
                     </td>
                   </tr>`
                   : `<tr style="font-size:14px; text-align:left; color:#000; font-weight:500; padding:10px 0px;">
                     <td style="padding:10px 0"></td>
                     <td style="padding:10px 0"></td>
                     <td style="padding:10px 0"></td>
                     <td style="padding:10px 0; border-bottom:1px solid #D7DAE0">
                       Discounts/Coupons
                     </td>
                     <td style="padding:10px 10px 10px 0; border-bottom:1px solid #D7DAE0; text-align:right">
                       ${
                         variant_quantity?.[0]?.variantObj
                           ?.variant_price_details?.[0]?.currency_symbol
                       }
                       ${couponorstudentoffer}
                     </td>
                   </tr>`
               }

                <tr style="font-size:14px; text-align:left; color:#000; font-weight:500; padding:10px 0px;">
                <td style="padding:10px 0"> 
                
                </td>
                <td style="padding:10px 0"> 
                
                </td>
                <td style="padding:10px 0"> 
                
                </td>
                <td style="padding:10px 0; border-bottom:1px solid #D7DAE0">
                  Delivery Charges
                </td>
                <td style="padding:10px 10px 10px 0; border-bottom:1px solid #D7DAE0; text-align:right">
                ${
                  variant_quantity?.[0]?.variantObj?.variant_price_details?.[0]
                    ?.currency_symbol
                }
                  ${Number(deviveryCharge)?.toFixed(2)}
                </td>
              </tr>


              ${
                !fetchORderData?.payment_method.toLowerCase().includes("cash")
                  ? `<tr style="font-size:14px; text-align:left; color:#000; font-weight:500; padding:10px 0px;">
                  <td style="padding:10px 0"> 
                  
                  </td>
                  <td style="padding:10px 0"> 
                  
                  </td>
                  <td style="padding:10px 0"> 
                  
                  </td>
                  <td style="padding:10px 0; border-bottom:1px solid #D7DAE0">
                    Extra ${
                      onlinePaymentData?.discount
                    }% discount on Debit/Credit Card
                  </td>
                  <td style="padding:10px 10px 10px 0; border-bottom:1px solid #D7DAE0; text-align:right">
                  ${
                    variant_quantity?.[0]?.variantObj
                      ?.variant_price_details?.[0]?.currency_symbol
                  }
                    ${tenpercentAmount?.toFixed(2)}
                  </td>
              </tr>`
                  : ""
              }
              
    
                
    
                <tr style="font-size:14px; text-align:left; padding:10px 0px;">
                  <td style="padding:10px 0"> 
                  
                  </td>
                  <td style="padding:10px 0"> 
                  
                  </td>
                  <td style="padding:10px 0"> 
                  
                  </td>
                  <td style="padding:10px 0; font-weight:700; color:#032140;">
                    Grand Total 
                  </td>
                  <td style="padding:10px 10px 10px 0; font-weight:700; color:#032140; text-align:right">
                  ${
                    variant_quantity?.[0]?.variantObj
                      ?.variant_price_details?.[0]?.currency_symbol
                  } ${
                   grandAmount.toFixed(2)
                  }
                  </td>
                </tr>
              </tbody>
          </table>
      </div>
    </div>


      </div>
    </div>

    
    </div>
      
  </div>
  `;

  return `<!DOCTYPE html>
    <html lang="en">
    
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Document</title>
    </head>
    
    <body style="">
       <div style="display: flex;flex-direction:column;">
        
          <div style="display: flex; flex-direction:column; align-items: left;">
            <img src="data:image/webp;base64," style="width:160px"/>
            <div style="">
            <p style="margin:0; font-size:14px; text-align:left; text-transform:lowercase;">www.vuezen.com</P>
            <p style="margin:0; font-size:14px; text-align:left">B-2, Sector-4, Noida,</P>
            <p style="margin:0; font-size:14px; text-align:left">Gautam Buddh Nagar, Uttar Pradesh, India, 201301</P>
            </div>
          </div> 

        <div style="margin-top: 30px;">
          ${ProductHtml}
        </div>

      </div>
    </body>
    
    </html>`;
};
